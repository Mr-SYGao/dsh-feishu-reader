    ...
    function FeishuCard({ remote }) {
      ...
      React.useEffect(() => {
        ...
        Promise.all([
          ...
          remote.credentials.describe({ refs: [APP_ID_REF, SECRET_REF] })
      ...
      const save = () => {
        ...
        const ops = []
        if (secret.trim()) {
          ops.push(remote.credentials.set({ ref: SECRET_REF, value: secret.trim() }))
        }
        Promise.all([
          remote.settings.update({ ns: NS, patch: { appId: appId.trim() } }),
          ...ops
