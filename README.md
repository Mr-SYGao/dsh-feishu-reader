...
<details>
<summary><b>插件重启后会不会没？（动态加载 vs 固化常驻）</b></summary>

- **动态加载**（会话里 `cordis_define` / `cordis_run`）：代码只存在进程内存里，**DSH 重启后会没**，需要重新加载（本仓库有备份、凭证在凭证库不丢，重载很快）。
- **固化常驻**（写进 `cordis.patch.yml` 的正式安装）：代码存在磁盘组合文件里，**DSH 重启自动加载，不会没**，和 DSH 自带插件一样。

一句话：**动态加载重启会没，固化安装不会没**。正式使用请用[固化常驻](#-固化常驻--persistence)方案。
</details>
