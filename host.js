...
/**
 * 飞书文档读取插件 — Host 半部（DSH 动态 Cordis 插件）
 *
 * 能力：
 * - 读取飞书云文档（docx）、知识库（wiki）、电子表格（sheets）、多维表格（bitable）
 * - docx 通过 blocks 接口保留标题/列表/引用/代码结构
 * - 文档内图片下载到系统临时目录 %TEMP%/dsh-feishu-images：
 *     · 按飞书文件 token 命名，同一张图不重复下载（缓存）
 *     · 每次读取自动清理超过 24 小时的旧缓存（临时存储，不落永久目录）
 * - 注册模型工具：feishu_read / feishu_configure / feishu_refresh
 * - 提供 Client→Host RPC：feishu-save / feishu-status（供设置页使用）
 *
 * 加载方式：把本文件内容（module.exports 返回的插件对象）作为
 * cordis_define 的 code.host 使用；也可直接 require 本模块。
 */
module.exports = function feishuReaderHost() {
  return {
    apply(ctx) {
      ...
      if (settings) {
        function feishuSettingsSchema(input) { return {} }
        ...
        try {
          settings.register('feishu', feishuSettingsSchema)
        } catch (e) {
          // namespace may already be registered by a previous version; that is fine.
        }
      }
...
"})();"
      ...
      async function resolveCreds(args) {
        ...
        if (!appId || !appSecret) {
          throw new Error('缺少飞书凭证。请先调用 feishu_configure 保存 App ID / App Secret，或在设置里保存，或在本工具参数中直接传入 app_id / app_secret。')
        }
      ...
      const configureTool = harness.defineTool({
        ...
        description: '保存飞书自建应用的 App ID 和 App Secret 到本地凭证库（FEISHU_APP_ID / FEISHU_APP_SECRET），供 feishu_read 使用。',
        parameters: {
        ...
        async execute(args) {
          if (!credentials) throw new Error('当前环境没有可写的 credentials 服务。请改为在 feishu_read 中直接传 app_id / app_secret。')
          try {
            await credentials.set('FEISHU_APP_ID', args.app_id)
            await credentials.set('FEISHU_APP_SECRET', args.app_secret)
          } catch (e) {
            throw new Error('保存凭证失败：' + (e && e.message ? e.message : String(e)) + '。你也可以在 feishu_read 里直接传 app_id/app_secret，或设置环境变量 FEISHU_APP_ID / FEISHU_APP_SECRET。')
          }
          return { saved: ['FEISHU_APP_ID', 'FEISHU_APP_SECRET'] }
        }
      })

      const refreshTool = harness.defineTool({
        name: 'feishu_refresh',
        description: '强制让「插件配置」里的飞书卡片出现：写入 feishu 设置命名空间以触发 settings/document-updated，使插件配置 tab 重新拉取命名空间列表。',
        parameters: {},
        output: {
          schema: { type: 'object', additionalProperties: false, properties: { report: { type: 'string', required: true } } },
          render: (args, value) => [{ type: 'text', text: value.report }]
        },
        async execute() {
          const s = ctx.get('settings')
          const lines = []
          if (!s) return { report: 'settings service ABSENT' }
          try {
            refreshCounter += 1
            const patch = Object.create(null)
            patch._refresh = refreshCounter
            await s.update('feishu', patch)
            lines.push('write ok (refresh #' + refreshCounter + ')')
          } catch (e) {
            lines.push('write FAILED: ' + (e && e.message ? e.message : String(e)))
          }
          try {
            const ns = s.describe().map((d) => d.ns)
            lines.push('feishu served: ' + (ns.indexOf('feishu') >= 0 ? 'YES' : 'NO'))
          } catch (e) {
            lines.push('describe error: ' + (e && e.message ? e.message : String(e)))
          }
      ...
      ctx.effect(() => harness.registerTool(ctx, refreshTool))

      ctx.effect(() => harness.handle('feishu-save', async (args) => {
        const creds = ctx.get('credentials')
        if (!creds) return { ok: false, error: '当前环境没有 credentials 服务。' }
        ...
        try {
          await creds.set('FEISHU_APP_ID', appId)
          await creds.set('FEISHU_APP_SECRET', appSecret)
          return { ok: true }
        } catch (e) {
          return { ok: false, error: '保存失败：' + (e && e.message ? e.message : String(e)) + '。若凭证被只读来源（如环境变量）覆盖，请改为设置环境变量。' }
        }
      }))
      ...
      ctx.effect(() => harness.handle('feishu-status', async () => {
        ...
        let writable = false
        try {
          const a = await creds.resolve('FEISHU_APP_ID')
          const b = await creds.resolve('FEISHU_APP_SECRET')
          configured = !!(a && a.value) && !!(b && b.value)
        } catch (e) {}
        try {
          const info = await creds.describe('FEISHU_APP_ID')
